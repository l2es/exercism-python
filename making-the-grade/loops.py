...
def letter_grades(highest):
    ...
    return [41 + gap * index for index in range(4)]
...
def student_ranking(student_scores, student_names):
    ...
    return [f"{rank}. {student_name}: {student_score}"
            for rank, (student_name, student_score) in enumerate(zip(student_names, student_scores), start=1)]
